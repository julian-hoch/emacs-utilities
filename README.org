...
This function allows me to move the current buffer to the next window.
...
(defun jh/move-buffer-to-other-window ()
  ...
  (let ((current-buffer (current-buffer))
    ...
    (if (eq original-window next-window)
        (message "No other window.")
      (switch-to-buffer-other-window current-buffer)
      (set-window-buffer original-window (other-buffer current-buffer))
      (select-window original-window))))
#+end_src

#+RESULTS:
: jh/move-buffer-to-other-window
